...
class Lexer:
    ...
    def tokenize(self) -> List[Tuple[str, Any]]:
        ...
        """
        #Maps all the strings to the type of token it is 
        keywords = {'if':'IF','else': 'ELSE', 'for': 'FOR','print':'PRINT','not':'NOT','or': 'OR','and': 'AND', 'to': 'TO'}
        #Make the empty list for tokens 
        tokens = []
        for match in re.finditer(self.token_regex, self.code):
            ...
            #Make a number to INT
            elif token_type == 'NUMBER':
                
                tokens.append((token_type, int(token_value)))
            #Check to see keywords 
            elif token_type == 'IDENTIFIER':
                if token_value in keywords: 
                    tokens.append((keywords[token_value], token_value))
                else:
...
class Parser:
    ...
    def parse_statement(self) -> ASTNode:
        ...
        #the variable's assignment, x=5, etc
        ...
        #token is unknown 
            ...
            raise SyntaxError(f"Token has an unexpected token in statement: {token_type} at position {self.pos}")
    # TODO: Implement this function
    def parse_if_stmt(self) -> IfStatement:
        ...
        """
        #check for 'IF' as the keyword 
        self.expect('IF')
        #Parse our condition 
        condition = self.parse_boolean_expression()
        #Check for colon for after the condition 
        self.expect('COLON')
        #parse If block
        if_block = self.parse_block()
        #check  if the is an else 
        else_block = None
        if self.current_token()[0] == 'ELSE':
            self.expect('ELSE')
            #chechk for a colon after else 
            self.expect('COLON')
            #parse the else 
            else_block = self.parse_block()
        #Create & return an if statement node     
    ...
    def parse_for_stmt(self) -> ForStatement:
        ...
        self.expect('FOR')
        #get our loop variable name 
        lpvar = self.expect("IDENTIFIER")
        
        #check for an equals sign
        self.expect('EQUALS')
        #parse our starting value 
        st_expr = self.parse_boolean_expression()
        #check for the 'TO' keyword 
        self.expect('TO')
        
        #Parse the ending value 
        ed_exp = self.parse_boolean_expression()
        self.expect('COLON')
        #Loop 
        body = self.parse_block()
        #return for loop node 
        return ForStatement(lpvar, st_expr, ed_exp, body)
    ...
    def parse_print_stmt(self) -> PrintStatement:
        ...
        #Start the empty argument list 
        ag = []
        #Check for arguments 
        if self.current_token()[0] != "RPAREN":
            #parse the argument
            ag = self.parse_arg_list()
        #check fo )    
        self.expect('RPAREN')
        
        return PrintStatement(ag)
    ...
    def parse_block(self) -> Block:
        ...
        """
        #Empty list for statements 
        sttmts = []
        #Parse until else of end of file is reached 
        while self.current_token()[0] not in ('ELSE', 'EOF'): 
            token_type = self.current_token()[0]
            #Check for valid statement time 
            if token_type in ('IDENTIFIER', 'IF', 'FOR','PRINT'):
                #parse, add the statement 
                sttmts.append(self.parse_statement())
            else:
                #stop parsing 
                break
        return Block(sttmts)
    ...
    def parse_arg_list(self) -> List[ExprType]:
        ...
        ag.append(self.parse_boolean_expression())
    ...
    def parse_boolean_expression(self) -> ExprType:
        ...
        """
        #parse first term 
        l = self.parse_boolean_term()
        #parse or statements 
        while self.current_token()[0] == 'OR':
            #check for 'or' operator 
            op = self.expect('OR')
            #parse right side 
            r = self.parse_boolean_term()
            #make into a logic op 
            l = LogicalOperation(l, op, r)
    ...
    def parse_boolean_term(self) -> ExprType:
        ...
        """
        #parse first factor 
        l = self.parse_boolean_factor()
        
        #will keep parsing And operate 
        while self.current_token()[0] == 'AND':
            #check for and op 
            op = self.expect('AND')
            #will parse the right 
            r = self.parse_boolean_factor()
            #will combine logic into a operation 
            l = LogicalOperation(l, op, r)
    ...
    def parse_boolean_factor(self) -> ExprType:
        ...
        """
        #checks for a not operation 
        if self.current_token()[0] == 'NOT':
            #checks for the not operator 
            op = self.expect('NOT')
            #parse what will come after the NOT
    ...
    def parse_factor(self) -> ExprType:
        ...
        if self.current_token()[0] in ('PLUS', 'MINUS'):
            ...
            #move to next token 
            self.advance()
            #parse what comes after our operator 
            operand = self.parse_factor()
            #return unary operation 
            return UnaryOperation(op, operand)
        #go back to parse a primary expression 
    ...
    def parse_primary(self) -> ExprType:
        ...
        #Error 
            ...
            raise SyntaxError(f"The Primary has an unexpected token in: {token_type} at {self.pos}")
